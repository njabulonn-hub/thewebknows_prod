...
accuracy:"This value is <strong><span class=\"tag-known\">known</span></strong> because it is the exact IPv4 address your connection broadcasts to every site you visit.",
...
accuracy:"This is <strong><span class=\"tag-known\">known</span></strong> when your ISP enables IPv6. If nothing appears, your provider is not broadcasting IPv6 to the web.",
...
accuracy:"This information is <strong><span class=\"tag-known\">known</span></strong> and drawn from public routing databases that map IP ranges to ISPs.",
...
accuracy:"This is <strong><span class=\"tag-guess\">inferred</span></strong> from commercial geolocation databases and can be wrong, especially when you use VPNs or corporate routes.",
...
accuracy:"Referrer values are <strong><span class=\"tag-known\">known</span></strong> because browsers send them automatically unless a privacy control removes the header.",
...
accuracy:"This is an educated <strong><span class=\"tag-guess\">guess</span></strong> based on naming patterns and may mislabel niche or newly announced networks.",
...
accuracy:"It is <strong><span class=\"tag-known\">known</span></strong> only if your ISP publishes a reverse DNS record; many residential IPs do not have one.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> because your browser reports it, unless a privacy tool rewrites the string. Modern browsers may provide high-entropy hints only to trusted origins using User-Agent Client Hints negotiation.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> because your browser shares it, and even when spoofed many sites can still guess correctly using feature detection and TLS fingerprinting.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> because it comes straight from your browser settings. Developers know that HTTP negotiation falls back through the language list until a supported locale is found.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> because it comes from your device clock. Programmers often treat it as the browser's resolved IANA timezone name, which updates automatically as you travel.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> and calculated directly from your timezone settings, though skewed system clocks can throw it off. Engineers sync devices with NTP or chrony to keep offsets within milliseconds.",
...
accuracy:"This is <strong><span class=\"tag-guess\">GUESSED</span></strong> and can be fooled by unusual screen setups or developer tools. It typically uses heuristics like pointer precision, screen diagonal, and hardware concurrency.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> because your browser reports the real size unless you are faking it, though multi-monitor scaling can cause subtle shifts. Front-end engineers account for devicePixelRatio and OS scaling when interpreting results.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> and updates in real time as you resize, sourced from the rendering engine's layout tree. Developers use ResizeObserver and matchMedia to react to the same numbers.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> because it comes from your hardware, although zoom levels and OS scaling can tweak it slightly. Canvas rendering APIs reveal the same ratio when measuring backing store pixels.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> because it reports your real display capability, though virtualization layers can cap it. Graphics programmers inspect the same value when tuning WebGL contexts and canvas rendering.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> because the browser reads it straight from your hardware, though some browsers round it for privacy. Kernel-level APIs expose the same logical processor count for native apps.",
...
accuracy:"This is <strong><span class=\"tag-guess\">ESTIMATED</span></strong> because many browsers share only rounded buckets such as 4 GB or 8 GB. Internally it is derived from hardware telemetry but intentionally coarsened for privacy.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> and usually exact unless the browser masks the vendor. Developers rely on the same info in graphics debugging consoles and driver compatibility checks.",
...
accuracy:"This is <strong><span class=\"tag-guess\">GUESSED</span></strong> because some browsers hide the full list until you have granted permission before, and device IDs may be randomized. Media APIs expose only hashed identifiers unless the origin is trusted.",
...
accuracy:"This is <strong><span class=\"tag-guess\">GUESSED</span></strong> because many browsers reveal only limited information and may obfuscate labels. Browser makers sometimes require secure contexts or user gestures to expose full device metadata.",
...
accuracy:"This is <strong><span class=\"tag-guess\">GUESSED</span></strong> because many browsers hide the exact count until permission is granted, and identifiers may rotate. Under privacy sandboxing, device IDs are ephemeral to stop cross-site tracking.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> because the browser knows if the feature is available, usually only on secure pages and some embedded browsers. Engineers confirm it via feature detection or permissions.query().",
...
accuracy:"This is <strong><span class=\"tag-known\">MEASURED</span></strong> and can be very close, especially on phones with GPS, assisted GNSS, or Wi-Fi triangulation. Engineers inspect accuracy and timestamp fields to gauge sensor quality.",
...
malicious:"It's a minor factor in device fingerprinting.",
accuracy:"This is <strong><span class=\"tag-guess\">GUESSED</span></strong> and can be wrong if drivers misreport touch support or if remote desktops translate input. Web runtimes often cache maxTouchPoints and fall back to zero when uncertain.",
...
accuracy:"This is <strong><span class=\"tag-guess\">GUESSED</span></strong> based on what the browser knows, aggregating device capabilities and fallback heuristics.",
...
accuracy:"This is <strong><span class=\"tag-guess\">ESTIMATED</span></strong> because it only tests a limited list and some browsers block the check entirely, returning generic metrics. Privacy-first browsers randomize or disable local font enumeration.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> because the browser simply says yes, no, or unsupported, mapping to plugin availability.",
...
accuracy:"This is <strong><span class=\"tag-guess\">GUESSED</span></strong> because many browsers block the list or show only built-in entries, making the data incomplete. Chromium-based browsers now gate plugin enumeration behind feature policies.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> only when the browser shares it, and many do not, because the specification is still experimental. Engineers treat the value as advisory since desktop browsers often return \"unknown.\"",
...
accuracy:"This is <strong><span class=\"tag-guess\">ESTIMATED</span></strong> based on recent activity and can change quickly, especially as radio conditions fluctuate. Developers know it is derived from moving averages of RTT and downlink measurements.",
...
accuracy:"This is <strong><span class=\"tag-guess\">ESTIMATED</span></strong> and updates as your real-world speed changes, but it is not a certified speed test. Browser engines smooth the value using rolling averages to avoid constant jitter.",
...
accuracy:"This is <strong><span class=\"tag-guess\">ESTIMATED</span></strong> from recent network activity and can fluctuate with congestion. Engineers often compare it against ICMP ping or synthetic monitoring results for validation.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> because it comes straight from your browser setting and does not change mid-session unless you toggle it.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> because it is either on, off, or unset and is read directly from browser settings.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> when your browser supports it and sets the value to true.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> because the browser knows whether cookies are allowed, taking into account third-party restrictions, SameSite rules, and partitioned storage.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> because the browser decides whether the feature is available, respecting private-browsing modes and storage partitions.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> from your system settings and updates instantly when you toggle the theme at the OS or browser level.",
...
accuracy:"This is <strong><span class=\"tag-known\">KNOWN</span></strong> because it comes from your system accessibility settings and updates instantly when you toggle the option.",
